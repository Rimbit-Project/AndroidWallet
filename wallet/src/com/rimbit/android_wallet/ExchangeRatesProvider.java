...
package com.rimbit.android_wallet;
...
import com.rimbit.android_wallet.util.Io;
...
public class ExchangeRatesProvider extends ContentProvider
{
	...
	private static final URL RIMBIT_EXPLORER_URL;
	private static final String[] RIMBIT_EXPLORER_FIELDS = new String[] { "24h_avg", "last" };
	private static final String RIMBIT_EXPLORER_SOURCE = "rimbitexplorer.com";

	// https://bitmarket.eu/api/ticker

	static {
		try {
			RIMBIT_EXPLORER_URL = new URL("https://rimbitexplorer.com/q/getrimbitprice");
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final String userAgent) {
		...
		String usdPriceString = getURLResult(RIMBIT_EXPLORER_URL, userAgent);
		if (usdPriceString == null)
			return null;

		BigInteger usdPrice = GenericUtils.parseCoin(usdPriceString, 0);
		rates.put("USD", new ExchangeRate("USD", usdPrice, RIMBIT_EXPLORER_SOURCE));
		...
		if (yahooRes != null) {
			try {
				...
				for (int x = 0; x < convs.length(); x++) {
					...
					BigInteger price = (new BigDecimal(GenericUtils.parseCoin(priceString, 0).multiply(usdPrice))).movePointLeft(6).toBigIntegerExact();
					rates.put(currency, new ExchangeRate(currency, price, RIMBIT_EXPLORER_SOURCE));
